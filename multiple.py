...
def main():
    x=['m2.csv','m4.csv', 'm5.csv']
    print ("first file")
    ifile=open('/home/bushra/Desktop/icp/multiple/m1.csv','r')
    ...
    for w in range(0,3):
        print ("second file")
        ifile1=open('/home/bushra/Desktop/icp/multiple/'+ x[w],'r')
        ...
        C=np.mat(B)*np.mat(R1)+np.mat(t1.T)
        ...
        my_data = genfromtxt('/home/bushra/Desktop/icp/multiple/m1.csv', delimiter=',')
        points2X = my_data[:,0]
        points2Y = my_data[:,1]
        points2Z = my_data[:,2]
    ## I remove the header of the CSV File.
    ...
    #ax.plot(points1X, points1Y, points1Z, 'd', markersize=8, markerfacecolor='red', label='points1')
        ax.plot(points2X, points2Y, points2Z, color = 'blue', alpha = 0.5, marker ='o', markersize=5, label='points2')

        a = np.asarray(C)
        np.savetxt("foo"+str(w)+".csv", a, delimiter=",")

        my_data = genfromtxt("foo"+str(w)+".csv", delimiter=',')
        points3X = my_data[:,0]
        points3Y = my_data[:,1]
        points3Z = my_data[:,2]
        '''
        ...
        points3Y = np.delete(points2Y, 0)f
        ...
        ax.plot(points3X, points3Y, points3Z,color = color[w],alpha = 0.5, marker = mark[w], markersize=5, label='points3')
    plt.show()
